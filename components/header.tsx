...
export function Header() {
  ...
  return (
    ...
    >
      {/* FULL WIDTH CONTAINER - NO INNER MAX-WIDTH */}
      <div className="w-full px-6 md:px-10 flex items-center justify-between relative">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`px-5 py-2.5 rounded-none text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 relative group overflow-hidden ${
                    isActive 
        ...
        <div className="hidden md:flex items-center z-20">
          ...
          >
            <span className="text-[10px] font-black uppercase tracking-[0.2em] hidden xl:block text-right">Chat with us</span>
            <div className="w-12 h-12 rounded-none bg-black/5 border border-black/5 flex items-center justify-center group-hover:bg-[#C97C2F] group-hover:text-white transition-all duration-300">
           ...
           <div className="flex flex-col gap-8">
            {navLinks.map((link) => (
              <Link
                ...
                className="text-4xl font-black text-[#111827] hover:text-[#C97C2F] transition-colors uppercase tracking-tighter border-b border-black/5 pb-8"
              >
                {link.name}
              </Link>
            ))}
