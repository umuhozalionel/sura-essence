  ...
  return (
    <section id="pricing" className={`pt-24 pb-8 bg-[#F9F8F6] text-[#0A1128] relative overflow-hidden ${manrope.className}`}>
      ...
      <div className="max-w-[1600px] mx-auto px-10 relative z-10">
          ...
          <div className="flex flex-col gap-3 min-w-[340px] w-full lg:w-auto">
             {perks.map((perk, idx) => {
                ...
                <motion.div 
                   ...
                   viewport={{ once: true }}
                   className="flex items-center gap-4 p-5 bg-white rounded-2xl ring-1 ring-black/5 shadow-lg group hover:border-[#125740] transition-all"
                >
                   <Icon className="w-5 h-5 text-[#125740]" />
                   <span className="text-xs font-black text-[#0A1128] uppercase tracking-widest">{perk.label}</span>
                </motion.div>
                );
             })}
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-0 rounded-3xl ring-1 ring-black/5 shadow-2xl shadow-black/10 overflow-hidden">
          {services.map((item, i) => (
            <motion.div
              key={item.id}
              initial={{ opacity: 0 }}
              ...
              viewport={{ once: true }}
              className="group bg-white p-8 border-r border-black/5 last:border-r-0 hover:bg-[#0A1128] transition-all duration-500 flex flex-col h-full"
            >
               ...
               <div className="flex flex-col flex-grow">
                  <h3 className="text-2xl font-black text-[#0A1128] group-hover:text-white uppercase tracking-tighter mb-4 transition-colors">
                     {item.title}
                  </h3>
                  <p className="text-sm font-bold text-[#0A1128]/50 group-hover:text-white/50 mb-8 leading-relaxed uppercase tracking-tight">
                     {item.description}
                  </p>
                  ...
                  <div className="space-y-4 mb-10">
                     {item.features.map((feature) => (
                        <div key={feature} className="flex items-center gap-3">
                           <div className="w-1.5 h-1.5 rounded-full bg-[#125740]" />
                           <span className="text-[10px] font-black text-[#0A1128]/60 group-hover:text-white/60 uppercase tracking-[0.2em]">{feature}</span>
                        </div>
                     ))}
                  </div>
        ...
        >
           <Link href="/book" className="group flex flex-col items-end">
              <span className="text-[11px] font-black text-[#0A1128] uppercase tracking-[0.4em] mb-2 flex items-center gap-3">
                {t("cta")} <ArrowRight className="w-4 h-4 text-[#125740] group-hover:translate-x-2 transition-transform" />
              </span>
              <div className="h-[2px] w-48 bg-[#125740]" />
           </Link>
        </motion.div>
