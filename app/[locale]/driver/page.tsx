...
function DriverContent() {
  ...
  const getDayNightStatus = useMemo(() => {
    ...
    if (hour >= 18 || hour < 5) return { label: t("status.evening"), icon: Moon, color: "text-blue-400" };
    return { label: t("status.day"), icon: Sun, color: "text-[#C97C2F]" };
  }, [kigaliTime, t]);
    ...
    <main className={`min-h-screen bg-[#F5F2EA] ${manrope.className} selection:bg-[#C97C2F]/20 relative`}>
      ...
      <section className="relative pt-48 pb-32 px-10 bg-[#111827] overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-6xl md:text-9xl font-black text-white mb-10 leading-[0.85] tracking-tighter uppercase">
            {t("title")} <br /><span className="text-[#C97C2F]">{t("titleHighlight")}</span>
          </motion.h1>
      ...
      <section className="py-32 px-10 max-w-[1600px] mx-auto relative z-20">
         <div className="mb-20 max-w-2xl border-l-4 border-[#C97C2F] pl-8">
            <h2 className="text-5xl font-black text-[#111827] uppercase tracking-tighter mb-4">{t("standards.title")}</h2>
         ...
         <div className="grid md:grid-cols-4 gap-1 border border-gray-200 bg-gray-200 shadow-2xl">
            {standards.map((feature) => {
              ...
              return (
              <div key={feature.id} className="bg-white p-12 group hover:bg-[#111827] transition-all duration-500">
                <div className="w-16 h-16 bg-[#F5F2EA] group-hover:bg-[#C97C2F]/10 flex items-center justify-center mb-10 transition-colors">
                  <Icon className="w-8 h-8 text-[#C97C2F]" />
                </div>
      ...
      <section id="fleet" className="py-32 px-10 max-w-[1600px] mx-auto relative z-20 bg-white border border-gray-200 shadow-2xl">
         <div className="grid lg:grid-cols-12 gap-20 items-start">
            <div className="lg:col-span-4">
                <span className="text-[#C97C2F] text-[11px] font-black uppercase tracking-[0.4em] mb-4 block">{t("fleet.badge")}</span>
                <h2 className="text-6xl font-black text-[#111827] uppercase tracking-tighter mb-8 leading-[0.9] whitespace-pre-line">{t("fleet.title")}</h2>
            ...
            <div className="lg:col-span-8 grid md:grid-cols-2 gap-1 bg-gray-200 border border-gray-200 overflow-hidden">
               {fleet.map((car) => {
                 ...
                 return (
                 <div key={car.id} className="bg-white group hover:bg-[#111827] transition-all duration-500 flex flex-col">
                    {/* CAR IMAGE CONTAINER */}
                    ...
                    <div className="p-10 flex flex-col h-full">
                        <div className="flex items-center gap-4 mb-6">
                           <CarIcon className="w-6 h-6 text-[#C97C2F]" />
      ...
      <section className="py-40 px-10 max-w-[1600px] mx-auto text-center">
          <h2 className="text-5xl font-black text-[#111827] uppercase tracking-tighter mb-20">{t("reviews.title")}</h2>
                  ...
                  <div className="relative z-10">
                     <div className="flex items-center gap-6 mb-10">
                        ...
                        <div>
                           ...
                           <span className="text-[10px] font-black text-[#C97C2F] uppercase tracking-[0.3em]">{t("reviews.rental", { car: rev.car })}</span>
                        </div>
                     </div>
                     <p className="text-lg font-bold text-[#111827] italic leading-relaxed uppercase tracking-tight mb-0">{t("reviews.quote", { text: rev.comment })}</p>
                     {reviewNote && (
                       <p className="text-[9px] font-bold uppercase tracking-widest text-[#111827]/30 mt-3">{reviewNote}</p>
                     )}
                  </div>
               </div>
             ))}
...
export default function DriverPage() {
  const t = useTranslations("Driver");
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#111827] flex items-center justify-center text-white font-black uppercase tracking-[0.5em]">{t("loading")}</div>}>
      <DriverContent />
    </Suspense>
  );
}