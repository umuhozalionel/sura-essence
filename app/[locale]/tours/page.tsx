...
function ToursContent() {
  ...
  const missionStatus = useMemo((): { label: string; icon: LucideIcon; color: string } => {
    ...
    if (hour >= 17 && hour < 18) return { label: ts("goldenHour"), icon: Sun, color: "text-[#EAB308]" };
    if (hour >= 18 || hour < 5) return { label: ts("night"), icon: Moon, color: "text-[#EAB308]" };
    return { label: ts("optimal"), icon: Zap, color: "text-[#125740]" };
  }, [kigaliTime, ts]);
      ...
      <div className="relative z-10">
        ...
        {(activeCategory || activeVenue) && (
            <div className="fixed top-24 left-6 z-[60]">
                <button onClick={handleBack} aria-label={t("back")} className="flex items-center justify-center w-14 h-14 bg-white text-[#0A1128] hover:bg-[#0A1128] hover:text-white shadow-xl border border-gray-100 transition-all">
                    ...
                    <div className="flex-1 space-y-12">
                        {activeVenue.missionIntel && (
                            <div className="flex flex-wrap gap-4">
                                <div className="bg-[#0A1128] text-white px-4 py-2 border-l-2 border-[#125740] flex items-center gap-2">
                                    <TrafficCone className="w-3 h-3 text-[#125740]" /><span className="text-[10px] font-black uppercase tracking-widest">{activeVenue.missionIntel.driveTime}</span>
                                </div>
                                <div className="bg-[#0A1128] text-white px-4 py-2 border-l-2 border-[#125740] flex items-center gap-2">
                                    <Activity className="w-3 h-3 text-[#125740]" /><span className="text-[10px] font-black uppercase tracking-widest">{t("vibe", { vibe: activeVenue.missionIntel.vibe })}</span>
                                </div>
                                <div className="bg-[#0A1128] text-white px-4 py-2 border-l-2 border-[#125740] flex items-center gap-2">
                                    <Sun className="w-3 h-3 text-[#125740]" /><span className="text-[10px] font-black uppercase tracking-widest">{t("target", { time: activeVenue.missionIntel.bestTime })}</span>
                                </div>
                            </div>
                        )}
                        <div>
                               ...
                               {activeVenue.expect?.map((item) => {
                                  ...
                                  return (
                                    <div key={item.id} className="bg-white border border-gray-200 p-6 flex items-center gap-5 shadow-sm">
                                        <div className="w-10 h-10 bg-[#F9F8F6] flex items-center justify-center text-[#125740]"><Icon size={20} /></div>
                                        <span className="text-[10px] font-black uppercase tracking-widest text-[#0A1128]">{item.label}</span>
                                    </div>
                                  );
                               })}
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 border border-gray-200 bg-white shadow-2xl">
                                {[{ icon: Clock, label: activeVenue.duration }, { icon: Users, label: t("maxGroup", { size: activeVenue.groupSize }) }, { icon: Check, label: t("age", { age: activeVenue.minAge }) }, { icon: Star, label: t("ratingLabel", { rating: activeVenue.rating }) }].map((s, i) => (
                                    <div key={i} className="p-8 border-r last:border-r-0 border-gray-100 flex flex-col items-center gap-2"><s.icon className="w-5 h-5 text-[#125740]" /><span className="text-[10px] font-black text-[#0A1128] uppercase tracking-widest text-center">{s.label}</span></div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="max-w-[1600px] mx-auto px-10 py-32 relative z-10">
                    <div className="mb-20 max-w-4xl border-l-4 border-[#125740] pl-8">
                        {activeCategory.intro.desc.map((l, i) => (
                            <p key={i} className="text-2xl md:text-4xl font-black text-[#0A1128] uppercase tracking-wide leading-tight block mb-2">{l}</p>
                        ))}
                    </div>
                                ...
                                <div className="h-[320px] relative overflow-hidden">
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                                        <div className="flex gap-4 items-center">
                                            <div className="flex items-center gap-2"><Clock size={12} className="text-[#125740]" /><span className="text-[9px] font-black text-white uppercase tracking-widest">{v.duration}</span></div>
                                            <div className="flex items-center gap-2"><MapPin size={12} className="text-[#125740]" /><span className="text-[9px] font-black text-white uppercase tracking-widest">{v.location}</span></div>
                                        </div>
                                    </div>
                                </div>
                                <div className="p-10 flex flex-col h-full">
                                    <h3 className="text-3xl font-black text-[#0A1128] group-hover:text-white uppercase tracking-tighter mb-8 transition-colors leading-none">{v.title}</h3>
                                    ...
                                    <div className="mt-auto pt-8 border-t border-gray-100 group-hover:border-white/10">
                                        <div className="h-14 w-full bg-[#0A1128] group-hover:bg-[#125740] flex items-center justify-between px-8 transition-all">
                                            <span className="text-[10px] font-black text-white uppercase tracking-[0.4em]">{t("exploreVenue")}</span>
                                            <ArrowRight size={16} className="text-[#125740] group-hover:text-white transition-transform group-hover:translate-x-2" />
                                        </div>
                                    </div>
                                </div>
                            </Link>
                            ...
                            <div className="relative max-w-lg text-right self-end min-h-[250px] p-0" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
                                <AnimatePresence mode="wait">
                                    <motion.div key={quoteIndex} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 1 }} className="flex flex-col items-end">
                                        <Quote className="w-10 h-10 text-[#125740] mb-6 ml-auto opacity-60" /><p className="text-xl font-bold text-white leading-tight mb-8 italic uppercase tracking-widest opacity-90 max-w-md">“{quotes[quoteIndex].text}”</p>
                                        <div className="flex flex-col items-end border-t border-white/10 pt-8 opacity-60"><p className="text-[#EAB308] font-black text-2xl tracking-tighter mb-2">{quotes[quoteIndex].author}</p><p className="text-white font-black text-[10px] uppercase tracking-[0.3em]">{quotes[quoteIndex].team}</p></div>
                                    </motion.div>
                                </AnimatePresence>
                            </div>
                        </div>
        ...
        <AnimatePresence>
            {isGalleryOpen && activeVenue && (
                <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col items-center justify-center p-6">
                    <button onClick={() => setIsGalleryOpen(false)} aria-label={tg("close")} className="absolute top-10 right-10 text-white/50 hover:text-[#125740] z-[120]"><X size={48} /></button>
...
export default function ToursPage() {
  ...
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-[#0A1128] flex items-center justify-center text-white font-black uppercase tracking-[0.5em]">
          {t("loading")}
        </div>
      }
    >
